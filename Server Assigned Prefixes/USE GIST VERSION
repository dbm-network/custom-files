...
Bot.init = function() {
	...
	console.log(g("You can access the prefix you set on the server by using"),y('${server.prefix}'), g("in the actions"))
	console.log(r("Disclamer: If DBM gets updated and the original bot.js is modifed by a dev"));
	console.log(r("this file WILL break your bot when ran, please let staff on the DBM Mods discord server"))
	console.log(r("know this so it can be updated to the latest. Use 'node bot.js' if this is so."))
	
	try {
		this.prefixes = require(Actions.getLocalFile(require("path").join("data", "serverPrefixes.json")))
	} catch (error) {
		console.log(r("\nserverPrefixes.json is missing from the 'data' directory"),g("\nUsing tag defined in (DBM)Settings.json for all servers."))
	}
...
Bot.onReady = function() {
	...
	if(this.prefixes.length > 0){
		for (var i = 0; i < this.prefixes.length; i++) { 
			...
			const server = this.prefixes[i].serverid;
			const tag = this.prefixes[i].prefix;
			const guild = this.bot.guilds.get(server);
		
			console.log(g('Using prefix "' + tag + '" on Server named "' + (guild.name || "Unknown") + '(' +server+ ')"'))
		}
	}
...
Bot.checkCommand = function(msg) {
	let command = this.checkTag(msg.content, msg.guild); // use this if prefixes.json is empty
...
Bot.checkTag = function(content, guild) {
	...
	if(prefixes.length > 0){
		...
		const server = guild;
		const usedTag = content.charAt(0);
			
		for (var i = 0; i < prefixes.length; i++) { 
			
			const rightServer = prefixes[i].serverid
			const rightTag = prefixes[i].prefix
			const canUse = rightServer === server.id && rightTag == usedTag;
																
			if(canUse){

				guild.prefix = usedTag;
				console.log("allowing '" + usedTag + "' on '" + rightServer);			
				if(content.startsWith(rightTag)) return content.substring(usedTag.length);
			}else{
				if(content.startsWith(tag) && canUse) {
